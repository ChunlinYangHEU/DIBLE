...
from disble import tools, blockencoding
...
def matrix_AB(non_zero_elements_A, non_zero_elements_B, num_ice, num_seawater):
    ...
    for n1 in range(num_ice):
        ...
        matrix_B[4 * n1 + 3, 4 * n1 + 3] = non_zero_elements_B[0]
        matrix_B[4 * n1 + 4, 4 * n1] = non_zero_elements_B[1]
        matrix_B[4 * n1 + 4, 4 * n1 + 4] = non_zero_elements_B[1]
        matrix_B[4 * n1 + 5, 4 * n1 + 2] = non_zero_elements_B[2]
        matrix_B[4 * n1 + 5, 4 * n1 + 5] = non_zero_elements_B[2]
    for n2 in range(num_seawater - 1):
...
if __name__ == '__main__':
    ...
    sound_velocity = 500  # Sound velocity at depth z, assumed to be constant
    frequency = 50  # Frequency of sound wave
    lambda_ice = 10  # Lamé coefficients of ice at depth z, assumed to be constant
    mu_ice = 10  # Lamé coefficients of ice at depth z, assumed to be constant
    parameters = {
        'depth_ice': depth_ice,
        'num_ice': num_ice,
